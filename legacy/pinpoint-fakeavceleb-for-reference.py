...
# =================================================================================
# 1. CONFIGURATION (### MODIFIED for Unified Preprocessing ###)
# =================================================================================
class Config:
    # --- Testing Flag ---
    ...
    # This should be the path to the folder containing your 'train', 'dev', and 'test' subdirectories.
    DATA_DIRECTORY = "/kaggle/input/fav-pp-xai-train/preprocessed_data" 
    
    # This should be the path to the single metadata file generated by your new script.
    # It's recommended to rename the generated file to something consistent like this.
    METADATA_PATH = "/kaggle/input/fav-pp-xai-train/preprocessed_data/preprocessed_metadata_small_sample_20250908_101542.json"
    ### END MODIFICATION ###
    ...
    CURRICULUM_PHASE1_EPOCHS = 2
    CURRICULUM_PHASE2_EPOCHS = 3
    ...
    ENABLE_SHAP = True
    SHAP_BACKGROUND_SAMPLES = 4
    SHAP_MAX_AUDIO_LEN = 400
    SHAP_MAX_VIDEO_FRAMES = 20
    ...
    ### START MODIFICATION ###
    # --- Data Preprocessing (Values from the new preprocessing script) ---
    NUM_FRAMES = 64  # MODIFIED: Changed from 30 to 64 to match the new script
    VIDEO_SIZE = (128, 128)
    NUM_MFCC = 13
    ### END MODIFICATION ###
...
class LAVDFDataset(Dataset):
    def __init__(self, config, split='train'):
        ...
        ### START MODIFICATION: Load the single unified metadata file ###
        if not os.path.exists(config.METADATA_PATH):
            raise FileNotFoundError(
                f"Unified metadata file not found at: {config.METADATA_PATH}\n"
                "Please run the new preprocessing script first and ensure the path is correct."
            )
            
        with open(config.METADATA_PATH, 'r') as f:
            all_metadata = json.load(f)
            
        # Filter the loaded metadata for the specific split ('train', 'dev', or 'test')
        ...
        # Construct full paths from the base directory and the relative paths in the metadata
        ...
        if self.config.TESTING:
            print(f"!!! TESTING FLAG IS TRUE: Truncating '{self.split}' split to 100 samples. !!!")
            random.shuffle(self.samples)
            self.samples = self.samples[:100]

        if not self.samples:
            raise ValueError(f"CRITICAL: No valid data samples found for the '{self.split}' split.")
    ...
    def __getitem__(self, idx):
        try:
            ...
            ### START FIX: Handle different audio tensor shapes ###
            ...
            if audio_tensor.ndim == 3:
                # Assuming shape is [13, 2, Time] -> average channels
                if audio_tensor.shape[0] == self.config.NUM_MFCC:
                    audio_tensor = audio_tensor.mean(dim=1)  # Average over channel dimension -> [13, Time]
                    audio_tensor = audio_tensor.transpose(0, 1)  # Transpose to [Time, 13]
                # Assuming shape is [2, 13, Time] -> average channels
                elif audio_tensor.shape[1] == self.config.NUM_MFCC:
                    audio_tensor = audio_tensor.mean(dim=0)  # Average over channel dimension -> [13, Time]
                    audio_tensor = audio_tensor.transpose(0, 1)  # Transpose to [Time, 13]
                # Assuming shape is [Time, Channels, 13]
                elif audio_tensor.shape[2] == self.config.NUM_MFCC:
                    audio_tensor = audio_tensor.mean(dim=1) # Average over channel dimension -> [Time, 13]
                else:
                    print(f"Warning: Unfixable 3D audio shape {original_shape} at {audio_path}. Skipping.")
                    return None
                # print(f"Reshaped 3D audio from {original_shape} to {audio_tensor.shape}")

            # Case 2: 2D tensor (might need transposing)
            elif audio_tensor.ndim == 2:
                # If shape is [13, Time], transpose to [Time, 13]
                if audio_tensor.shape[0] == self.config.NUM_MFCC:
                    audio_tensor = audio_tensor.transpose(0, 1)
                # If the second dimension isn't NUM_MFCC, it's a format error
                elif audio_tensor.shape[1] != self.config.NUM_MFCC:
