...
class LAVDFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # FIX: Added 'video_path' to the returned dictionary
            return {"video": video_tensor, "audio": audio_tensor, "label": label, "is_fake": is_fake, "offset_label": offset_label, "video_path": video_path}    
        except Exception as e:
...
def collate_fn(batch):
    """
    Custom collate function to handle variable-length audio, pad sequences,
    and carry over metadata like video_path.
    """
    ...
    config = Config() # Need a local config for constants
    
    # --- Robustness check for audio tensor shapes ---
    processed_batch = []
    expected_features = config.NUM_MFCC
    for item in batch:
        audio = item['audio']
        if not (audio.ndim == 2 and audio.shape[1] == expected_features):
            print(f"Warning: Skipping sample with unexpected audio shape: {audio.shape}. Expected 2D tensor with {expected_features} features.")
            continue
    ...
    video_paths = [] # <<< FIX: List to store video paths

    # --- Determine max audio length for padding ---
    ...
    for item in batch:
        ...
        # --- 2. Process Audio ---
        audio = item['audio']
        a_pad_len = max_audio_len - audio.shape[0]
        padded_a = F.pad(audio, (0, 0, 0, a_pad_len), "constant", 0)
        padded_audios.append(padded_a)

        # --- 3. Collect Labels and Metadata ---
        labels.append(item['label'])
        offset_labels.append(item['offset_label'])
        is_fakes.append(item['is_fake'])
        video_paths.append(item['video_path']) # <<< FIX: Add the video path to the list

    # --- 4. Stack and return the complete batch dictionary ---
    return {
        "video": torch.stack(padded_videos),
        "audio": torch.stack(padded_audios),
        "video_mask": torch.stack(video_masks),
        "label": torch.tensor(labels, dtype=torch.float32),
        "offset_label": torch.tensor(offset_labels, dtype=torch.long),
        "is_fake": torch.tensor(is_fakes, dtype=torch.bool),
        "video_path": video_paths  # <<< FIX: Include the list of paths in the output
    }
